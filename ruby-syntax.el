...
(defvar syntax-propertize-function nil
  ...
  ;; assume that the text before point has been propertized, so ruby-syntax-ppss
  ...
  ;; ruby-syntax-ppss-flush-cache since that would not only flush the cache but also
...
This function can call `ruby-syntax-ppss' on any position before END, but it
should not call `ruby-syntax-ppss-flush-cache', which means that it should not
call `ruby-syntax-ppss' on some position and later modify the buffer on some
earlier position.")
...
(defsubst ruby-syntax-ppss-depth (ppss)
  (nth 0 ppss))
...
(defsubst ruby-syntax-ppss-context (ppss)
  (cond
   ((nth 3 ppss) 'string)
   ((nth 4 ppss) 'comment)
   (t nil)))
